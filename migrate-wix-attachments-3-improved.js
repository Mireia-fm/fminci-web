// Script mejorado para migrar adjuntos de Wix desde Cargas del visitante-3
...
// Función mejorada para extraer información del nombre del archivo
...
async function migrateWixAttachments() {
  console.log('🚀 Iniciando migración MEJORADA de adjuntos desde Cargas del visitante-3...');
  ...
  try {
    ...
    // Extraer números de incidencia únicos de los nombres de archivos
    const incidenciaNumbers = new Set();
    mediaFiles.forEach(fileName => {
      const fileInfo = parseFileName(fileName);
      if (fileInfo.incidenciaNum) {
        incidenciaNumbers.add(fileInfo.incidenciaNum);
        // También agregar la versión sin padding
        if (fileInfo.originalNum) {
          incidenciaNumbers.add(fileInfo.originalNum);
        }
      }
    });

    console.log(`🔍 Números de incidencia únicos encontrados en archivos: ${incidenciaNumbers.size}`);
    console.log(`📋 Algunos ejemplos: ${Array.from(incidenciaNumbers).slice(0, 10).join(', ')}`);

    // 2. Obtener TODAS las incidencias que coincidan con los números encontrados
    console.log('🔍 Cargando incidencias que coincidan con archivos...');

    const incidenciaNumbersArray = Array.from(incidenciaNumbers);
    const { data: incidencias, error: incidenciasError } = await supabase
      ...
      .in('num_solicitud', incidenciaNumbersArray)
    ...
    console.log(`📊 Incidencias encontradas que coinciden: ${incidencias?.length || 0}`);
    ...
    incidencias?.forEach(inc => {
      ...
      // También mapear sin padding por si acaso
      incidenciasMap.set(inc.num_solicitud.replace(/^0+/, '') || inc.num_solicitud, inc);
    ...
    for (const fileName of mediaFiles) {
      try {
        ...
        // Intentar match por hash primero
        if (fileInfo.hash) {
        ...
        // Intentar match por número de incidencia (tanto con padding como sin padding)
        if (!targetIncidencia && fileInfo.incidenciaNum) {
          // Intentar con el número formateado
          if (incidenciasMap.has(fileInfo.incidenciaNum)) {
            targetIncidencia = incidenciasMap.get(fileInfo.incidenciaNum);
          }
          // Intentar con el número original sin padding
          else if (fileInfo.originalNum && incidenciasMap.has(fileInfo.originalNum)) {
            targetIncidencia = incidenciasMap.get(fileInfo.originalNum);
          }
          ...
          if (targetIncidencia) {
            ...
            if (isPDF) {
              ...
              if (!targetIncidencia.imagen_url || targetIncidencia.imagen_url.includes('wix:')) {
    ...
    console.log(`⏭️ Archivos ya existían: ${skipCount}`);
    console.log(`❌ Errores: ${errorCount}`);
    console.log(`📊 Ratio de match: ${((matchedCount/processedCount)*100).toFixed(1)}%`);

    if (errorCount === 0) {
      console.log('\n🎉 ¡Migración mejorada completada exitosamente!');
    } else {
