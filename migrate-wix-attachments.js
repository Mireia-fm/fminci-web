// Script para migrar adjuntos de Wix haciendo match con URLs
...
const wixFolder = '/Users/mireia/Downloads/Cargas del visitante-2';
...
async function migrateWixAttachments() {
  console.log('🚀 Iniciando migración de adjuntos de Wix...');
  ...
  try {
    ...
    const { data: incidencias, error: incidenciasError } = await supabase
      ...
      .limit(100); // Procesar las 100 más recientes
    ...
    const { data: comentarios, error: comentariosError } = await supabase
      .from('comentarios')
      .select('id, incidencia_id, imagen_url, documento_url')
    ...
    for (const fileName of mediaFiles) { // Procesar todos los archivos
      try {
        console.log(`\n📄 Procesando: ${fileName}`);
        ...
        if (fileInfo.hash) {
          if (incidenciasMap.has(fileInfo.hash)) {
            targetIncidencia = incidenciasMap.get(fileInfo.hash);
            fileCategory = 'imagen_principal';
            updateTarget = { tipo: 'incidencia', id: targetIncidencia.id };
            console.log(`  🎯 Match por hash en incidencia: ${targetIncidencia.num_solicitud}`);
          } else if (comentariosMap.has(fileInfo.hash)) {
