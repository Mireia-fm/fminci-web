...
async function checkPdfsVisitante3() {
  console.log('🔍 Verificando coincidencias de PDFs entre comentarios y visitante-3...\n');

  // 1. Obtener archivos PDF de visitante-3
  const allFilesPdf3 = fs.readdirSync(DOWNLOADS_PATH_3)
    .filter(file => /\.pdf$/i.test(file));

  console.log(`📁 Archivos PDF en visitante-3: ${allFilesPdf3.length}`);

  // 2. Obtener comentarios con documento_url que contengan URLs Wix (sin los ya migrados)
  ...
  console.log(`🔗 Comentarios con PDFs Wix restantes: ${comentariosPdf.length}\n`);
  ...
  for (const comentario of comentariosPdf) {
    ...
    const exactMatch = allFilesPdf3.find(file => file === wixFileName);
    if (exactMatch) {
    ...
    for (const file of allFilesPdf3) {
  ...
  console.log(`\n📊 RESULTADOS - COINCIDENCIAS DE PDFs EN VISITANTE-3:`);
  ...
  console.log('\n📈 RESUMEN FINAL:');
  console.log('='.repeat(80));
  console.log(`📁 Total archivos PDF visitante-3: ${allFilesPdf3.length}`);
  ...
  return {
    ...
    totalArchivos: allFilesPdf3.length,
    totalComentarios: comentariosPdf.length
  };
}

checkPdfsVisitante3().catch(console.error);